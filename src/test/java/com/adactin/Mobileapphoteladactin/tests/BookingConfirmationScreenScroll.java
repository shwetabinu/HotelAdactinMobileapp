...
/** 
 * 
 * Test case to verify scrolling in booking confirmation screen
 * @author shwetabinu
 *
 */
public class BookingConfirmationScreenScroll extends BaseClass{
	
	public BookingConfirmationScreenScroll() throws Exception {
		super();
	...
	/**
	 * Test case to verify if a user can scroll in the booking confirmation screen
	 * 
	 *@throws Exception Exception during test execution
	 */
	@Test(groups = { "functionalTest" })
	public void User_is_able_to_scroll_up_and_down_the_Booking_confirmation_screen() throws Exception
	{
		Log.startTestCase("User_is_able_to_scroll_up_and_down_the_Booking_confirmation_screen");
		int rno;
		//Reading the test case row number where the data is to be read
		ExcelUtil.setExcelFileSheet("Testcases");
		rno=ExcelUtil.readExcel('r',"User_is_able_to_scroll_up_and_down_the_Booking_confirmation_screen");
		...
		if((loginresult && searchresult && search && selecthotel
				&& selected_hotel && booknow && bookresult && book_confirm  
				)==true)
			ExcelUtil.setCellData("PASSED", rno, 0);
		else
			ExcelUtil.setCellData("FAILED", rno, 0);
		Log.endTestCase("User_is_able_to_scroll_up_and_down_the_Booking_confirmation_screen");
